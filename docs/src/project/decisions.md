...
```python
# Sans-IO core — pure logic, no async
class SessionManager:
    ...
    def check_timeouts(
        self, now: float,
    ) -> tuple[list[SessionExpired], float | None]:
        """Expire idle sessions.  Returns (expired, next_wakeup)."""
        ...

# I/O layer — one task, sleeps until next wakeup
async def timeout_loop(manager: SessionManager):
    while True:
        now = time.monotonic()
        expired, next_wakeup = manager.check_timeouts(now)
        for event in expired:
            ...  # clean up resources
        if next_wakeup is None:
            await signal.wait()  # no sessions, wait for one
        else:
            await asyncio.sleep(max(0, next_wakeup - time.monotonic()))
```
